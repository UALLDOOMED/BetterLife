 ...
 *
 * @uses kids_love_header_style()
 */
function kids_love_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'kids_love_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/custom-header-image.jpg',
		'default-text-color'     => 'fff',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'kids_love_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'kids-love' ),
		),
	) );
}
add_action( 'after_setup_theme', 'kids_love_custom_header_setup' );

if ( ! function_exists( 'kids_love_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see kids_love_custom_header_setup().
	 */
	function kids_love_header_style() {
		...
		wp_add_inline_style( 'kids-love-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'kids_love_header_style', 10 );
