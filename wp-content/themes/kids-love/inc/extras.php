 ...
 * @return array
 */
function kids_love_body_classes( $classes ) {
	...
	if ( $options['introduction_section_enable'] && is_front_page() ) {
	...
	if ( is_singular() || is_home() ) {
		$id = ( is_home() && ! is_front_page() ) ? get_option( 'page_for_posts' ) : get_the_id();
	  	$sidebar = get_post_meta( $id, 'kids-love-selected-sidebar', true );
...
add_filter( 'body_class', 'kids_love_body_classes' );
