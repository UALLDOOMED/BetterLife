...
registerBlockType( 'cgb/block-ilist-block', {
	...
	edit: function( props ) {
		...
		function showShortcodeModal(e){
			 $('#ilist_shortcode_generator_meta').prop('disabled', true);
			 $(e.target).addClass('currently_editing');
			$.post(
				ajaxurl,
				{
				...
				function(data){
					//console.log($(data).find('#qcsld_add_shortcode').attr('value'));
					if($('#ilist-modal').length<1){
						$('#wpwrap').append(data);
					}
		...
		function insertShortCode(e){
			const shortcode = $('#wpwrap').find('.ilist_shortcode_modal #ilist_shortcode_container').val();
			setAttributes( { shortcode: shortcode } );
			console.log(shortcode);
		}

		$(document).on('click','.ilist_copy_close', function(e){
			e.preventDefault();
			jQuery('.currently_editing').next('#ilist_insert_shortcode').trigger('click');
			$(document).find( '.ilist_shortcode_modal .ilist-modal-content .close').trigger('click');
			//const shortdata = $(this).attr('short-data');
			//setAttributes( { shortcode: shortdata } );
		});

		$(document).on( 'click', '.ilist_shortcode_modal .ilist-modal-content .close', function(){
			$('.currently_editing').removeClass('currently_editing');
			$('#ilist_shortcode_generator_meta').prop('disabled', false);
